...
module PipelineWatcher
  class CodebuildStatusWatcher
    def initialize(config, credential_manager = nil)
      ...
      @codebuild_client, @sts_client = @credential_manager.create_codebuild_clients
      @build_states = {}
    end
    ...
    def start_watching
      ...
      puts "AWS CodeBuild Watcher - Monitoring #{@config['codebuild_project_names'].size} project(s)".colorize(:cyan)
    ...
    def display_initial_screen
      ...
      # Display header
      puts "AWS CodeBuild Watcher - Last updated: #{Time.now.strftime('%Y-%m-%d %H:%M:%S')}".colorize(:cyan)
      puts '=' * 80
      puts
      ...
      @config['codebuild_project_names'].each do |project_name|
        update_build_display(project_name)
      end
    ...
    def update_header_timestamp
      move_cursor_to(1, 1)
      clear_line
      print "AWS CodeBuild Watcher - Last updated: #{Time.now.strftime('%Y-%m-%d %H:%M:%S')}".colorize(:cyan)
    end
    ...
    def update_build_display(project_name)
      begin
        ...
        build_info = @build_data[project_name]
        if build_info[:last_display] != new_display
          update_build_lines(project_name, new_display)
          build_info[:last_display] = new_display
        end
      rescue StandardError => e
        error_display = format_error_display(project_name, e.message)
        build_info = @build_data[project_name]
        if build_info[:last_display] != error_display
          update_build_lines(project_name, error_display)
          build_info[:last_display] = error_display
        end
      end
    end
    ...
    def update_build_lines(project_name, display_data)
      ...
      # Update first line (build status)
      move_cursor_to(row, 1)
      clear_line
      print display_data[:line1]

      # Update second line (phase info)
    ...
    def get_current_phase_info(build)
      if build.current_phase
        ...
        when 'FAILED', 'FAULT', 'TIMED_OUT'
          error_details = get_failure_details(build)
    ...
    def refresh_aws_credentials_runtime
      @codebuild_client, @sts_client = @credential_manager.refresh_runtime_codebuild_clients(@codebuild_client, @sts_client)
      @credential_manager.credentials_valid?(@sts_client)
    end

    def get_failure_details(build)
      details = []
